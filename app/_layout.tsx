...
export default function RootLayout() {
  ...
  useEffect(() => {
    ...
    if (!inCustomerApp && !inConsultantApp) {
      console.log("Redirecting to login page for variant:", APP_VARIANT);
      if (APP_VARIANT === "customer") {
    ...
    if (APP_VARIANT === "customer" && inConsultantApp) {
      console.log(
        "Customer variant but in consultant app - redirecting to customer",
      );
      router.replace("/(customer)/login");
    } else if (APP_VARIANT === "consultant" && inCustomerApp) {
      console.log(
        "Consultant variant but in customer app - redirecting to consultant",
      );
      router.replace("/(consultant)/login");
    }
  }, [fontsLoaded, fontError, segments, APP_VARIANT]);
